...
export const auditRequestSchema = z.object({
  ...
  mode: z.enum(["live", "benchmark", "test", "manual"]).default("live"), // Gap 1: live vs benchmark mode; "test" = mock responses; "manual" = queue prompts for manual paste
...
// Manual prompt queue — stores prompts for manual paste mode
export const manualPromptQueue = pgTable("manual_prompt_queue", {
  id: serial("id").primaryKey(),
  auditId: integer("audit_id").notNull(),
  engine: text("engine").notNull(), // e.g., "gemini", "chatgpt", "claude", "grok", "perplexity"
  query: text("query").notNull(),
  systemPrompt: text("system_prompt").notNull(),
  status: text("status").notNull().default("pending"), // 'pending' | 'completed'
  response: text("response"), // null until pasted back
  createdAt: timestamp("created_at").notNull().defaultNow(),
